...
let validateRecord ({ RecordId=id; ParentId=parentId } as record) =
    ...
    let idDifferParentId =
        ...
        if (id <> 0 && parentId <> 0) then
            if (id = parentId)
                then failwith (sprintf "Direct cycle was detected for record: %A" record)
...
module TreeBuilder =
    ...
    let tryToPlaceLeaf treeBuilder { RecordId=recId; ParentId=parentId } =
        ...
        if treeBuilder.waitingParents.ContainsKey parentId then
            ...
            if recId = 0 then
                let afterRoot = waitings.[0]
                match afterRoot with
                | Branch (_, children) -> children := add (Leaf recId) !afterRoot.children
                | _                    -> let newWaitings = Branch(recId, ref waitings)
